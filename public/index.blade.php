        ...
        <div class="col-sm-3 col-xs-5" id="menuwrapper">
            ...
            @if (count($categories) > 0)
                @foreach ($categories as $category)
                    <li><a href="{{ url('categories', str_slug($id = $category->id)) }}">{{ $category->name }}</a>
                        ...
                        <ul style="z-index:51; overflow:auto;">
                                            ...
                                            @if(Auth::check())             
                                                    ...
                                                    <form action="{{ url('category', $category->id) }}" method="POST" id="delete" style="display:inline">
                                                            {{ csrf_field() }}
                                                            {{ method_field('DELETE') }}
                                                            <button type="submit" id="delete-category-{{ $category->id }}" style="position:absolute; top:59%; right:76%; margin-top:4%; margin-right:10%;" id="delete" class="deletebtn">&times;</button>
                                                    </form>
                                                    </span>
                                            @endif
                            </li>
